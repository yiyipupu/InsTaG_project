...
class GaussianModel:
    ...
    def __init__(self, args):
        ...
        # 添加SH系数平滑相关变量
        self.prev_features_dc = None
        self.prev_features_rest = None
        self.sh_smooth_weight = 0.01  # SH平滑权重
        self.sh_energy_weight = 0.001  # SH能量正则化权重

        self.setup_functions()

    def compute_sh_smoothing_loss(self):
        """计算SH系数平滑损失"""
        loss = 0.0

        # 时间一致性损失
        if self.prev_features_dc is not None:
            # SH直流分量平滑
            loss += self.sh_smooth_weight * torch.mean(
                (self._features_dc - self.prev_features_dc) ** 2
            )

            # SH高频分量平滑
            if self._features_rest.shape[1] > 0 and self.prev_features_rest is not None:
                loss += self.sh_smooth_weight * torch.mean(
                    (self._features_rest - self.prev_features_rest) ** 2
                )

        # 能量正则化（防止SH系数过大）
        loss += self.sh_energy_weight * torch.mean(self._features_dc ** 2)
        if self._features_rest.shape[1] > 0:
            loss += self.sh_energy_weight * torch.mean(self._features_rest ** 2)

        return loss

    def update_sh_history(self):
        """更新SH历史记录"""
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_identity, new_opacities,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_identity, new_opacity, new_scaling,
                                   new_rotation)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_identity, new_opacities,
                                   new_scaling, new_rotation)
